...
- **스케일링 상수(s)**: 50
- **정수화 분모(denom)**: 120
- **플레인텍스트 모듈러스**: 593779228673 (약 39.1비트, NTT-friendly 소수)
- **곱셈 뎁스**: 6
- **링 차원**: 8192
- **보안 레벨**: HEStd_NotSet

**⚠️ 중요: 128비트 보안 제한**
5차 근사에서는 128비트 보안을 유지할 수 없습니다. 128비트 보안을 시도할 경우 다음과 같은 오류가 발생합니다:
```
terminate called after throwing an instance of 'lbcrypto::OpenFHEException'
what(): Change parameters! Try reducing the number of additions per level, number of key switches per level, or the digit size. We cannot support moduli greater than 60 bits.
```

이는 5차 근사의 복잡성(6 뎁스)과 128비트 보안 요구사항이 결합되어 60비트 모듈러스 제한을 초과하기 때문입니다. 따라서 NotSet 보안 레벨을 사용하여 성능을 최적화했습니다.
...
**파라미터:**
- PlaintextModulus: 593779228673 (약 39.1비트)
- PlaintextModulus/2: 296889614336
- ic1: 750000000, ic3: -50000, ic5: 1

**실제 값 비교 (180도 예시):**
- term1_raw: 117750000000 (약 19.8% of PlaintextModulus)
- term2_raw: -193494650000 (약 -32.6% of PlaintextModulus)
- term3_raw: 95388992557 (약 16.1% of PlaintextModulus)
- **결론**: 5차 근사는 모듈러스의 33%까지 사용하여 더 큰 값 처리
...
- **5차 근사의 한계**: 6 뎁스의 복잡성과 128비트 보안 요구사항이 결합되면 60비트 모듈러스 제한을 초과하여 실행 불가능

### 해결책:
- **링 차원 축소**: 16384 → 8192로 줄여서 모듈러스 크기 감소
- **보안 레벨 조정**: 5차 근사에서는 NotSet 사용으로 성능 최적화
...
| 항목 | 3차 근사 | 5차 근사 |
|------|----------|----------|
| **최대 term 크기** | ~3.5M (47.3%) | ~193M (32.6%) |
| **모듈러스 사용률** | 중간 | 중간 |
| **안전 마진** | 충분함 | 충분함 |
| **스케일링 효율성** | 중간 | 중간 |
...
2. **5차 근사**: 모듈러스의 33%까지 사용하지만 여전히 안전
...
-90         -1.570796       -0.922615 -1.000000 0.077385    -1185000     493039      -1185000     493039      8.03        23.46       2.61        34.10
0           0.000000        0.000000  0.000000  0.000000    0           0           0           0           8.54        28.01       3.27        39.82
90          1.570796        0.922615  1.000000  0.077385    1185000     -493039     1185000     -493039     6.81        25.66       2.44        34.91
180         3.141593        2.019857  0.000000  2.019857    2355000     3470140     2355000     3470140     6.89        23.12       2.63        32.65
```

### 5차 근사 결과
```
각도(도)    x_input(rad)    근사값    실제값    오차    term1_raw        term2_raw        term3_raw    term1_mod       term2_mod        term3_mod       암호화(ms)    연산(ms)    복호화(ms)    총시간(ms)
-90         -1.570796       -1.000000 -1.000000 0.000000    -59250000000    24651950000     -3077056399    -59250000000    24651950000     -3077056399    9.39        50.63       6.93        66.95
0           0.000000        0.000000  0.000000  0.000000    0             0             0             0             0             0             9.90        50.48       6.20        66.58
90          1.570796        1.000000  1.000000  0.000000    59250000000    -24651950000    3077056399    59250000000    -24651950000    3077056399     10.93       80.90       7.93        99.76
180         3.141593        0.523849  0.000000  0.523849    117750000000   -193494650000   95388992557   117750000000   -193494650000   95388992557    9.48        53.97       7.41        70.87
```
...
- **5차 근사 128비트 보안 제한**: 5차 근사에서는 128비트 보안을 유지할 수 없음 (60비트 모듈러스 제한 초과)
